...
namespace TxoutSet.Tests
{
    public class UnitTest1
    {
        ...
        public async Task CanPublishTweets()
        {
            ...
            var resp1234 = await _publisher.Client.SendAsync(publishTweetMsg(strBody, "1234"));
            Assert.True(resp1234.IsSuccessStatusCode);
            var resp4567 = await _publisher.Client.SendAsync(publishTweetMsg(strBody, "5678"));
            Assert.True(resp4567.IsSuccessStatusCode);

            await Task.Delay(3000);

            var console = await _publisher.Client.GetStringAsync("/api/ZTest");
        ...
        private HttpRequestMessage publishTweetMsg(string strBody, string apiKey)
        {
            return new HttpRequestMessage
            {
                Method = HttpMethod.Post,
                RequestUri = new Uri(PUBLISHER_ROOT + "/api/publish"),
                Headers = {
                        { "ApiKey", apiKey },
                        { "X-Version", "1" }
                    },
